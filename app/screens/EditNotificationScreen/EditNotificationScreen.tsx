...
import {ILoaderService} from '../../service/loader/loader-service-interface';
...
const EditNotificationScreen = observer(() => {
    ...
    const loaderService: ILoaderService = useInjection(Type.LoaderService);
    const loaderState: LoaderState = useInjection(Type.LoaderState);
    ...
    const loadingStatus = loaderState.getIsLoading();
    const [loading, setLoading] = useState(loadingStatus);
    useEffect(() => {
        setTimeout(() => {
            // loaderService.setLoader(!loading);
            setLoading(!loading);
        }, 500);
    }, []);
    ...
    return (
        <SafeAreaView style={styles.container}>
            ...
            {loading ? <ActivityIndicator size={'large'} color={'royalblue'} style={styles.indicator}/> : <View style={styles.mainWrapper}>
                <TouchableOpacity activeOpacity={0.8} onPress={() => navigation.navigate(Route.NOTIFICATIONS_SCREEN as never)}>
